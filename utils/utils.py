...
def lr_policy(lr_fn):
    '''
    服务于训练过程中学习率的调整，仅被lr_step_policy调用
    :param lr_fn:
    :return:一个函数，根据当前epoch，调整optimizer的学习率
    '''
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        res = []
        for k in topk:
            correct_k = correct[:k].contiguous().view(-1).float().sum(0)  # 加上了.contiguous()
